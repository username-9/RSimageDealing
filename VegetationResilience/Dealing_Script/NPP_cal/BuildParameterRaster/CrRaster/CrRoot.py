...
def para_cal(input_file_path, output_file_dir, ref_dictionary):
    ...
    CrRoot_path = os.path.join(output_file_dir, "y_" + os.path.basename(input_file_path)[9:13] + ".tif")
    driver = gdal.GetDriverByName('GTiff')
    CrRoot_ds: gdal.Dataset = driver.Create(CrRoot_path, ds_width, ds_height, bands=ds_band_num, eType=gdal.GDT_Float32)
    CrRoot_ds.SetGeoTransform(ds_trans)
    CrRoot_ds.SetProjection(ds_proj)
    sla_array = CrRoot_ds.GetRasterBand(1).ReadAsArray()
    # get reference parameter
    ref_s_dict = {}
    for i in range(8):
        if i == 7:
            ref_s_dict[str(i + 1)] = ref_dictionary[str(11)]["11"]
        else:
            ref_s_dict[str(i + 1)] = ref_dictionary[str(i + 1)]["11"]
    ...
    CrRoot_ds.WriteArray(lucc_array)
    CrRoot_ds.FlushCache()
    del CrRoot_ds
    print("y raster done")
...
if __name__ == "__main__":
    ...
    out_dir = r"D:\Data\VegetationResilienceDealing\Integrate_Output\PARAMETER\Rroot"
