...
def para_cal(input_file_path, output_file_dir, ref_dictionary):
    ...
    ds_datatype = in_ds.GetRasterBand(1).DataType
    ds_band_num = in_ds.RasterCount
    del in_ds
    # create empty SLA raster
    sla_path = os.path.join(output_file_dir, "SLA" + os.path.basename(input_file_path))
    driver = gdal.Driver("GTiff")
    sla_ds: gdal.Dataset = driver.Create(sla_path, ds_width, ds_height, bands=ds_band_num, eType=ds_datatype)
...
if __name__ == "__main__":
    ...
    ref_dict = {}
    json_path = r"C:\Users\PZH\PycharmProjects\RSimageDealing\Vegetation Resilience\lut.json"
    if os.path.exists(json_path):
        with open(json_path, 'r') as f:
            ref_dict = json.load(f)
    else:
        lut_path = r"C:\Users\PZH\Desktop\LUT.xlsx"
